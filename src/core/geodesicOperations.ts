...
class GeodesicOperations {
  ...
  private const earthRadius = 6371e3; // metres
  ...
  setCoordinateHemispheres(coordinate: Coordinate): Coordinate {

    if(coordinate.Latitude.decimal >= 0){

      coordinate.Latitude.hemisphere = Hemisphere.North;

    } else {      

      coordinate.Latitude.hemisphere = Hemisphere.South;

    }

    if(coordinate.Longitude.decimal >= 0) {

      coordinate.Longitude.hemisphere = Hemisphere.East;

    } else {
      
      coordinate.Longitude.hemisphere = Hemisphere.West;
  ...
  distanceHaversine(coordinate1: Coordinate, coordinate2: Coordinate): number {
    ...
    const Δφ =
      ((coordinate2.Latitude.decimal - coordinate1.Latitude.decimal) * Math.PI) /
      180;
    const Δλ =
      ((coordinate2.Longitude.decimal - coordinate1.Longitude.decimal) * Math.PI) /
      180;
  ...
  destinationFromBearing(coordinate1: Coordinate, bearing: number, distance: number): Coordinate {
    
    const φ2 = Math.asin( Math.sin(coordinate1.Latitude.decimal)*Math.cos(distance/this.earthRadius) +
                      Math.cos(coordinate1.Latitude.decimal)*Math.sin(distance/this.earthRadius)*Math.cos(bearing) );
    const λ2 = coordinate1.Longitude.decimal + Math.atan2(Math.sin(bearing)*Math.sin(distance/this.earthRadius)*Math.cos(coordinate1.Latitude.decimal),
                           Math.cos(distance/this.earthRadius)-Math.sin(coordinate1.Latitude.decimal)*Math.sin(φ2));

	  // The longitude can be normalised to −180…+180 using (lon+540)%360-180
    let latitude:Latitude
    let coordinate:Coordinate = new Coordinate(φ2,λ2);


    this.setCoordinateHemispheres();
